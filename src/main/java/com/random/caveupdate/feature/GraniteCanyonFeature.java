...
public class GraniteCanyonFeature extends Feature<DefaultFeatureConfig> {

	public GraniteCanyonFeature(Codec<DefaultFeatureConfig> configCodec) {
		super(configCodec);
			               ...
			               for(ac = 0; ac < 16; ++ac) {
			                  for(ad = 0; ad < 8; ++ad) {
			                     ...
			                     if (bl2) {
			                        ...
			                        if (ad < 4 && !material.isSolid() && serverWorldAccess.getBlockState(vertPos.add(ab, ad, ac)) != Blocks.CAVE_AIR.getDefaultState()) {
			                           return false;
			                        }
			                     }
			                  }
			               ...
			               for(ac = 0; ac < 16; ++ac) {
			                  for(ad = 0; ad < 8; ++ad) {
			                     if (bls[(ab * 16 + ac) * 8 + ad]) {
			                        serverWorldAccess.setBlockState(vertPos.add(ab, ad, ac), ad >= 4 ? Blocks.CAVE_AIR.getDefaultState() : Blocks.CAVE_AIR.getDefaultState(), 2);
			                     }
			                  }
			               }
			            }
			                     ...
			                     for(ad = 0; ad < 8; ++ad) {
			                        ...
			                        if (bl2 && (ad < 4 || random.nextInt(2) != 0) && serverWorldAccess.getBlockState(vertPos.add(ab, ad, ac)).getBlock() == Blocks.STONE) {
			                           if (vertPos.add(ab, ad, ac).getY() <= 18 && random.nextInt(6) == 0) {
			                        	   ...
			                        	   serverWorldAccess.setBlockState(vertPos.add(ab, ad, ac), Blocks.LAVA.getDefaultState(), 3);
			                        	   serverWorldAccess.getFluidTickScheduler().schedule(vertPos.add(ab, ad, ac), Blocks.LAVA.getDefaultState().getFluidState().getFluid(), 0);
			                           } else {
			                        	serverWorldAccess.setBlockState(vertPos.add(ab, ad, ac), Blocks.GRANITE.getDefaultState(), 2);
			                           }
			                        }
			                     }
			                  }
